...
例えば、「Ａ＝Ｂ」は「 LDA regA, STA regB」と２命令となり、各々２サイクルなので実行には４サイクル必要ですが、
２段パイプライン構成なので３サイクルとなり、１サイクルは20ns(50MHz)なので、60nsで実行されます。  
コンパイルはパソコン上もしくはＦＰＧＡ上で行うことが出来ます。  
パソコン無しでも操作可能であり、ＦＰＧＡ上でプログラムを入力後「#=1」を入力する事により実行されます。
